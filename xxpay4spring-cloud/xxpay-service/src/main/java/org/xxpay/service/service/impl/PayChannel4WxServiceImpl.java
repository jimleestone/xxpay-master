...
public class PayChannel4WxServiceImpl extends BaseService implements PayChannel4WxService {
	...
	public Map doWxPayReq(String jsonParam) {
		...
		try {
			...
			JSONObject payOrderObj = baseParam.isNullValue("payOrder") ? null
					: JSONObject.parseObject(bizParamMap.get("payOrder").toString());
			...
			try {
				...
				// return
				// XXPayUtil.makeRetData(XXPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS,
			...
			// return
			// XXPayUtil.makeRetFail(XXPayUtil.makeRetMap(PayConstant.RETURN_VALUE_FAIL,
