...
import org.xxpay.service.service.PayChannel4AliService;
import org.xxpay.service.service.PayChannel4WxService;
import org.xxpay.service.service.impl.BaseNotify4MchTrans;
import org.xxpay.service.service.impl.BaseService4TransOrder;
	...
	public void receive(String msg) {
		...
		if (transOrder.getStatus() != PayConstant.TRANS_STATUS_INIT) {
			_log.warn("转账状态不是初始({})或失败({}),不能转账.transOrderId={}", PayConstant.TRANS_STATUS_INIT,
					PayConstant.TRANS_STATUS_FAIL, transOrderId);
			return;
		}
		...
		if (isSuccess) {
			// 更新转账状态为成功
			String channelOrderNo = bizResult.get("channelOrderNo") == null ? ""
					: bizResult.get("channelOrderNo").toString();
			result = baseUpdateStatus4Success(transOrderId, channelOrderNo);
			_log.info("更新转账订单状态为成功({}),transOrderId={},返回结果:{}", PayConstant.TRANS_STATUS_SUCCESS, transOrderId,
					result);
			// 发送商户通知
			baseNotify4MchTrans.doNotify(transOrder, true);
		} else {
			// 更新转账状态为成功
			String channelErrCode = bizResult.get("channelErrCode") == null ? ""
					: bizResult.get("channelErrCode").toString();
			String channelErrMsg = bizResult.get("channelErrMsg") == null ? ""
					: bizResult.get("channelErrMsg").toString();
