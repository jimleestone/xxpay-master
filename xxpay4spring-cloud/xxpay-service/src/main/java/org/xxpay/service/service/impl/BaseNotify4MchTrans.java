...
import org.xxpay.service.mq.Mq4MchTransNotify;
...
public class BaseNotify4MchTrans extends BaseService4TransOrder {
	...
	public String createNotifyUrl(TransOrder transOrder, String backType) {
		...
		try {
			paramMap.put("device",
					URLEncoder.encode(ObjectUtils.defaultIfNull(transOrder.getDevice(), ""), PayConstant.RESP_UTF8));
			paramMap.put("param1",
					URLEncoder.encode(ObjectUtils.defaultIfNull(transOrder.getParam1(), ""), PayConstant.RESP_UTF8));
			paramMap.put("param2",
					URLEncoder.encode(ObjectUtils.defaultIfNull(transOrder.getParam2(), ""), PayConstant.RESP_UTF8));
	...
	public void doNotify(TransOrder transOrder, boolean isFirst) {
		...
		try {
			...
			_log.error(e, "transOrderId=%s,sendMessage error.",
					ObjectUtils.defaultIfNull(transOrder.getTransOrderId(), ""));
		}
	...
	public JSONObject createNotifyInfo(TransOrder transOrder, boolean isFirst) {
		...
		if (isFirst) {
			int result = baseInsertMchNotify(transOrder.getTransOrderId(), transOrder.getMchId(),
					transOrder.getMchTransNo(), PayConstant.MCH_NOTIFY_TYPE_TRANS, url);
