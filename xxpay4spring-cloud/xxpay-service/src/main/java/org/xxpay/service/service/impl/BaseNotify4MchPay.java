...
import org.xxpay.service.mq.Mq4MchPayNotify;
...
public class BaseNotify4MchPay extends BaseService4PayOrder {
	...
	public String createNotifyUrl(PayOrder payOrder, String backType) {
		...
		try {
			paramMap.put("device",
					URLEncoder.encode(ObjectUtils.defaultIfNull(payOrder.getDevice(), ""), PayConstant.RESP_UTF8));
			paramMap.put("subject",
					URLEncoder.encode(ObjectUtils.defaultIfNull(payOrder.getSubject(), ""), PayConstant.RESP_UTF8));
			paramMap.put("param1",
					URLEncoder.encode(ObjectUtils.defaultIfNull(payOrder.getParam1(), ""), PayConstant.RESP_UTF8));
			paramMap.put("param2",
					URLEncoder.encode(ObjectUtils.defaultIfNull(payOrder.getParam2(), ""), PayConstant.RESP_UTF8));
	...
	public JSONObject createNotifyInfo(PayOrder payOrder, boolean isFirst) {
		...
		if (isFirst) {
			int result = baseInsertMchNotify(payOrder.getPayOrderId(), payOrder.getMchId(), payOrder.getMchOrderNo(),
					PayConstant.MCH_NOTIFY_TYPE_PAY, url);
